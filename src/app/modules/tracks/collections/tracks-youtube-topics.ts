...
import {TracksYoutubeTopic} from '../models/tracks-youtube-topic';

export class TracksYoutubeTopics<TModel extends TracksYoutubeTopic> extends YoutubeCollection<TModel> {
  private static instance: TracksYoutubeTopics<TracksYoutubeTopic>;
  model: IModelConstructor = TracksYoutubeCategory;

  static getInstance(): TracksYoutubeTopics<TracksYoutubeTopic> {
    if (!this.instance) {
      this.instance = new TracksYoutubeTopics();
      this.instance.add([
        {
          id: '/m/04rlf',
          title: 'Music',
